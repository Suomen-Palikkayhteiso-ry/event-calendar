...
*/
						...
						return (
							...
							(function () {
								if (!context.b) {
									...
									return (
										' at json' + A2($elm$core$String$join, '', $elm$core$List$reverse(context))
									);
								}
							})()
						);
	...
	var $author$project$Events$FetchEvents = function (a) {
		return { $: 'FetchEvents', a: a };
	};
	...
	var $author$project$Calendar$init = {
	...
	var $author$project$EventForm$init = {
		allDay: false,
		description: '',
		endDate: '',
		errors: _List_Nil,
		...
		loading: false,
		location: '',
		mode: $author$project$EventForm$Create,
		point: $elm$core$Maybe$Nothing,
		startDate: '',
		state: $author$project$Types$Draft,
		title: '',
		url: ''
	};
	var $author$project$EventList$init = { currentPage: 1, pageSize: 20, showImportModal: false };
	var $author$project$Events$init = {
	...
	var $author$project$Main$Callback = { $: 'Callback' };
	var $author$project$Main$CreateEvent = { $: 'CreateEvent' };
	var $author$project$Main$EditEvent = function (a) {
		return { $: 'EditEvent', a: a };
	};
	var $author$project$Main$EventDetail = function (a) {
	...
	var $elm$url$Url$Parser$map = F2(function (subValue, _v0) {
		...
		return $elm$url$Url$Parser$Parser(function (_v1) {
			...
			return A2(
				...
				parseArg(A5($elm$url$Url$Parser$State, visited, unvisited, params, frag, subValue))
			);
		});
	});
	...
	var $author$project$Main$routeParser = $elm$url$Url$Parser$oneOf(
		_List_fromArray([
			...
			A2($elm$url$Url$Parser$map, $author$project$Main$MapRoute, $elm$url$Url$Parser$s('map')),
			A2(
			...
			A2(
				...
				A2($elm$url$Url$Parser$slash, $elm$url$Url$Parser$s('events'), $elm$url$Url$Parser$string)
			),
			A2(
				$elm$url$Url$Parser$map,
				$author$project$Main$EditEvent,
				A2(
					$elm$url$Url$Parser$slash,
					$elm$url$Url$Parser$s('events'),
					A2($elm$url$Url$Parser$slash, $elm$url$Url$Parser$string, $elm$url$Url$Parser$s('edit'))
				)
			),
			A2(
				$elm$url$Url$Parser$map,
				$author$project$Main$EventsRoute,
				$elm$url$Url$Parser$s('events')
			),
			A2($elm$url$Url$Parser$map, $author$project$Main$Callback, $elm$url$Url$Parser$s('callback'))
		])
	);
	...
	var $author$project$Events$EventDeleted = F2(function (a, b) {
	...
	var $author$project$PocketBase$authHeader = function (token) {
		if (token.$ === 'Just') {
			var t = token.a;
			return _List_fromArray([A2($elm$http$Http$header, 'Authorization', 'Bearer ' + t)]);
		} else {
			return _List_Nil;
		}
	};
	...
	var $elm_community$json_extra$Json$Decode$Extra$andMap =
	...
	var $elm_community$json_extra$Json$Decode$Extra$optionalField = F2(function (fieldName, decoder) {
	...
	var $author$project$Types$Point = F2(function (lat, lon) {
	...
	var $author$project$Types$pointEncoder = function (point) {
		return $elm$json$Json$Encode$object(
			_List_fromArray([
				_Utils_Tuple2('lat', $elm$json$Json$Encode$float(point.lat)),
				_Utils_Tuple2('lon', $elm$json$Json$Encode$float(point.lon))
			])
		);
	};
	...
	var $author$project$Types$eventEncoder = function (event) {
		return $elm$json$Json$Encode$object(
			_List_fromArray([
				_Utils_Tuple2('id', $elm$json$Json$Encode$string(event.id)),
				_Utils_Tuple2('title', $elm$json$Json$Encode$string(event.title)),
				_Utils_Tuple2('description', $author$project$Types$maybeStringEncoder(event.description)),
				_Utils_Tuple2('start_date', $elm$json$Json$Encode$string(event.startDate)),
				_Utils_Tuple2('end_date', $author$project$Types$maybeStringEncoder(event.endDate)),
				_Utils_Tuple2('all_day', $elm$json$Json$Encode$bool(event.allDay)),
				_Utils_Tuple2('url', $author$project$Types$maybeStringEncoder(event.url)),
				_Utils_Tuple2('location', $author$project$Types$maybeStringEncoder(event.location)),
				_Utils_Tuple2('state', $author$project$Types$eventStateEncoder(event.state)),
				_Utils_Tuple2('image', $author$project$Types$maybeStringEncoder(event.image)),
				_Utils_Tuple2(
					'image_description',
					$author$project$Types$maybeStringEncoder(event.imageDescription)
				),
				_Utils_Tuple2('point', $author$project$Types$maybePointEncoder(event.point)),
				_Utils_Tuple2('created', $elm$json$Json$Encode$string(event.created)),
				_Utils_Tuple2('updated', $elm$json$Json$Encode$string(event.updated))
			])
		);
	};
	...
	var $author$project$PocketBase$createEvent = F3(function (token, event, toMsg) {
		return $elm$http$Http$request({
			body: $elm$http$Http$jsonBody($author$project$Types$eventEncoder(event)),
	...
	var $elm$http$Http$expectWhatever = function (toMsg) {
		return A2(
			...
			toMsg,
			$elm$http$Http$resolve(function (_v0) {
				return $elm$core$Result$Ok(_Utils_Tuple0);
			})
		);
	};
	var $author$project$PocketBase$deleteEvent = F3(function (token, id, toMsg) {
		return $elm$http$Http$request({
	...
	var $author$project$PocketBase$getEvents = F2(function (token, toMsg) {
		return $elm$http$Http$request({
			body: $elm$http$Http$emptyBody,
			expect: A2(
				$elm$http$Http$expectJson,
				toMsg,
				$author$project$PocketBase$eventsResponseDecoder
			),
			headers: $author$project$PocketBase$authHeader(token),
			method: 'GET',
			timeout: $elm$core$Maybe$Nothing,
	...
	var $author$project$PocketBase$updateEvent = F4(function (token, id, event, toMsg) {
		return $elm$http$Http$request({
			body: $elm$http$Http$jsonBody($author$project$Types$eventEncoder(event)),
			expect: A2($elm$http$Http$expectJson, toMsg, $author$project$Types$eventDecoder),
			headers: $author$project$PocketBase$authHeader(token),
	...
	var $author$project$Events$update = F2(function (msg, model) {
		switch (msg.$) {
			case 'FetchEvents':
				var token = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { error: $elm$core$Maybe$Nothing, loading: true }),
					A2($author$project$PocketBase$getEvents, token, $author$project$Events$EventsFetched)
				);
			case 'EventsFetched':
				var result = msg.a;
				if (result.$ === 'Ok') {
					var events = result.a;
					return _Utils_Tuple2(
						_Utils_update(model, { events: events, loading: false }),
			...
			case 'CreateEvent':
				...
				return _Utils_Tuple2(
					model,
					A3(
						$author$project$PocketBase$createEvent,
						token,
						event,
						$author$project$Events$EventCreated
					)
				);
			case 'EventCreated':
				var result = msg.a;
			...
			case 'UpdateEvent':
				var token = msg.a;
				var id = msg.b;
				var updatedEvent = msg.c;
				return _Utils_Tuple2(
					model,
					A4(
						$author$project$PocketBase$updateEvent,
						token,
						id,
						updatedEvent,
						$author$project$Events$EventUpdated
					)
				);
			case 'EventUpdated':
				var result = msg.a;
	...
	var $author$project$Main$init = F3(function (flags, url, key) {
		var _v0 = A2(
			...
			$author$project$Events$FetchEvents($elm$core$Maybe$Nothing),
			$author$project$Events$init
		);
		var eventsModel = _v0.a;
		var eventsCmd = _v0.b;
		return _Utils_Tuple2(
			$author$project$Main$Model(key)(url)($author$project$Main$parseUrl(url))(
				$author$project$Calendar$init
			)(eventsModel)($author$project$Map$init)($author$project$EventForm$init)(
				$author$project$EventList$init
			)($elm$core$Maybe$Nothing)('')('')($elm$core$Maybe$Nothing)(true)(''),
		...
		A2(
			...
			function (user) {
				return A2(
					$elm$json$Json$Decode$andThen,
					function (token) {
						return $elm$json$Json$Decode$succeed({ token: token, user: user });
					},
					A2(
						$elm$json$Json$Decode$field,
						'token',
	...
	var $author$project$Ports$kmlContentParsed = _Platform_incomingPort(
		'kmlContentParsed',
	...
	var $author$project$Events$CreateEvent = F2(function (a, b) {
		return { $: 'CreateEvent', a: a, b: b };
	});
	var $author$project$Events$DeleteEvent = F2(function (a, b) {
		return { $: 'DeleteEvent', a: a, b: b };
	});
	...
	var $author$project$Events$UpdateEvent = F3(function (a, b, c) {
	...
	var $author$project$Types$Auth = F2(function (user, token) {
		return { token: token, user: user };
	});
	var $author$project$Types$User = F4(function (id, email, name, avatar) {
	...
	var $author$project$PocketBase$oauth2Encoder = F2(function (code, state) {
		return $elm$json$Json$Encode$object(
			_List_fromArray([
				_Utils_Tuple2('code', $elm$json$Json$Encode$string(code)),
				_Utils_Tuple2(
					'state',
					$elm$json$Json$Encode$string(A2($elm$core$Maybe$withDefault, '', state))
				)
			])
		);
	});
	...
	var $author$project$PocketBase$authWithOAuth2Code = F3(function (code, state, toMsg) {
		return $elm$http$Http$post({
			body: $elm$http$Http$jsonBody(A2($author$project$PocketBase$oauth2Encoder, code, state)),
	...
	var $author$project$PocketBase$loginEncoder = function (credentials) {
		return $elm$json$Json$Encode$object(
			_List_fromArray([
				_Utils_Tuple2('identity', $elm$json$Json$Encode$string(credentials.email)),
				_Utils_Tuple2('password', $elm$json$Json$Encode$string(credentials.password))
			])
		);
	};
	var $author$project$PocketBase$login = F2(function (credentials, toMsg) {
		return $elm$http$Http$post({
			body: $elm$http$Http$jsonBody($author$project$PocketBase$loginEncoder(credentials)),
			expect: A2($elm$http$Http$expectJson, toMsg, $author$project$PocketBase$authResponseDecoder),
			url: $author$project$PocketBase$baseUrl + '/api/collections/users/auth-with-password'
		});
	});
	var $author$project$PocketBase$logout = F2(function (token, toMsg) {
		return $elm$http$Http$request({
			body: $elm$http$Http$emptyBody,
			expect: $elm$http$Http$expectWhatever(toMsg),
			headers: _List_fromArray([A2($elm$http$Http$header, 'Authorization', 'Bearer ' + token)]),
			method: 'POST',
			timeout: $elm$core$Maybe$Nothing,
			tracker: $elm$core$Maybe$Nothing,
	...
	var $author$project$Main$parseCallbackParams = function (query) {
		var params = A2(
			$elm$core$List$filterMap,
			function (parts) {
			...
			A2($elm$core$List$map, $elm$core$String$split('='), A2($elm$core$String$split, '&', query))
		);
		var findParam = function (name) {
			return A2(
				$elm$core$Maybe$map,
		...
		var state = findParam('state');
		var code = findParam('code');
		return A2(
			$elm$core$Maybe$map,
			function (c) {
				return _Utils_Tuple2(c, state);
			},
			code
		);
	};
	var $author$project$Ports$parseKMLContent = _Platform_outgoingPort(
		'parseKMLContent',
		$elm$json$Json$Encode$string
	);
	var $author$project$DateUtils$daysSinceEpoch = F3(function (year, month, day) {
		var a = ((14 - month) / 12) | 0;
		var m = month + 12 * a - 3;
		var y = year + 4800 - a;
		var jd =
	...
	var $author$project$DateUtils$parseUTCDate = function (utcString) {
		var _v0 = A2($elm$core$String$split, 'T', utcString);
		if (_v0.b && _v0.b.b && !_v0.b.b.b) {
			var date = _v0.a;
			var _v1 = _v0.b;
			var timeWithZ = _v1.a;
			var time = A2($elm$core$String$dropRight, 1, timeWithZ);
			var _v2 = A2($elm$core$String$split, '-', date);
			if (_v2.b && _v2.b.b && _v2.b.b.b && !_v2.b.b.b.b) {
				...
				var _v4 = _v3.b;
				var dStr = _v4.a;
				var _v5 = A2($elm$core$String$split, ':', time);
				if (_v5.b && _v5.b.b && _v5.b.b.b && !_v5.b.b.b.b) {
					var hStr = _v5.a;
					var _v6 = _v5.b;
					var minStr = _v6.a;
					var _v7 = _v6.b;
					var sStr = _v7.a;
					var _v8 = $elm$core$String$toInt(yStr);
					if (_v8.$ === 'Just') {
						var y = _v8.a;
						var _v9 = $elm$core$String$toInt(mStr);
						if (_v9.$ === 'Just') {
							var m = _v9.a;
							var _v10 = $elm$core$String$toInt(dStr);
							if (_v10.$ === 'Just') {
								var d = _v10.a;
								var _v11 = $elm$core$String$toInt(hStr);
								if (_v11.$ === 'Just') {
									var h = _v11.a;
									var _v12 = $elm$core$String$toInt(minStr);
									if (_v12.$ === 'Just') {
										var min = _v12.a;
										var _v13 = $elm$core$String$toInt(sStr);
										if (_v13.$ === 'Just') {
											var s = _v13.a;
											var days = A3($author$project$DateUtils$daysSinceEpoch, y, m, d);
											var millis =
												days * 24 * 60 * 60 * 1000 +
												h * 60 * 60 * 1000 +
												min * 60 * 1000 +
	...
	var $author$project$KMLUtils$countryMap = $elm$core$Dict$fromList(
		_List_fromArray([
			_Utils_Tuple2('LAT', 'Latvia'),
			_Utils_Tuple2('EST', 'Estonia'),
			_Utils_Tuple2('LIT', 'Lithuania'),
			_Utils_Tuple2('FIN', 'Finland'),
			_Utils_Tuple2('SWE', 'Sweden'),
			_Utils_Tuple2('NOR', 'Norway'),
			_Utils_Tuple2('DNK', 'Denmark')
		])
	);
	var $author$project$DateUtils$localDateToUTC = function (localDateString) {
	...
	var $author$project$DateUtils$dateToString = function (_v0) {
		var day = _v0.day;
		var month = _v0.month;
		var year = _v0.year;
		return (
			...
			('-' +
				(A3($elm$core$String$padLeft, 2, _Utils_chr('0'), $elm$core$String$fromInt(month)) +
					('-' + A3($elm$core$String$padLeft, 2, _Utils_chr('0'), $elm$core$String$fromInt(day)))))
		);
	};
	var $author$project$KMLUtils$months = $elm$core$Dict$fromList(
		_List_fromArray([
			_Utils_Tuple2('january', 0),
			_Utils_Tuple2('jan', 0),
			_Utils_Tuple2('february', 1),
			_Utils_Tuple2('feb', 1),
			_Utils_Tuple2('march', 2),
			_Utils_Tuple2('mar', 2),
			_Utils_Tuple2('april', 3),
			_Utils_Tuple2('apr', 3),
			_Utils_Tuple2('may', 4),
			_Utils_Tuple2('june', 5),
			_Utils_Tuple2('jun', 5),
			_Utils_Tuple2('july', 6),
			_Utils_Tuple2('jul', 6),
			_Utils_Tuple2('august', 7),
			_Utils_Tuple2('aug', 7),
			_Utils_Tuple2('september', 8),
			_Utils_Tuple2('sep', 8),
			_Utils_Tuple2('october', 9),
			_Utils_Tuple2('oct', 9),
			_Utils_Tuple2('november', 10),
			_Utils_Tuple2('nov', 10),
			_Utils_Tuple2('december', 11),
			_Utils_Tuple2('dec', 11)
		])
	);
	var $elm$core$Basics$not = _Basics_not;
	var $elm$core$String$toLower = _String_toLower;
	var $elm$core$String$trim = _String_trim;
	var $author$project$KMLUtils$parseDateString = F2(function (dateStr, year) {
		var lower = $elm$core$String$toLower(dateStr);
		if (A2($elm$core$String$contains, 'mid', lower)) {
			var _v0 = A2(
				$elm$core$List$filter,
				A2($elm$core$Basics$composeL, $elm$core$Basics$not, $elm$core$String$isEmpty),
				A2($elm$core$String$split, ' ', lower)
			);
			if (_v0.b && _v0.b.b) {
				var _v1 = _v0.b;
				var monthName = _v1.a;
				var _v2 = A2($elm$core$Dict$get, monthName, $author$project$KMLUtils$months);
				if (_v2.$ === 'Just') {
					var month = _v2.a;
					var dateStr_ = $author$project$DateUtils$dateToString({
						day: 15,
						month: month + 1,
						year: year
					});
					return {
						endDate: $elm$core$Maybe$Just(dateStr_),
						startDate: $elm$core$Maybe$Just(dateStr_)
					};
				} else {
					return { endDate: $elm$core$Maybe$Nothing, startDate: $elm$core$Maybe$Nothing };
				}
			} else {
				return { endDate: $elm$core$Maybe$Nothing, startDate: $elm$core$Maybe$Nothing };
			}
		} else {
			if (A2($elm$core$String$startsWith, 'in ', lower)) {
				var _v3 = $elm$core$List$head(
					A2(
						$elm$core$String$split,
						' ',
						$elm$core$String$trim(A2($elm$core$String$dropLeft, 3, lower))
					)
				);
				if (_v3.$ === 'Just') {
					var monthName = _v3.a;
					var _v4 = A2($elm$core$Dict$get, monthName, $author$project$KMLUtils$months);
					if (_v4.$ === 'Just') {
						var month = _v4.a;
						var start = $author$project$DateUtils$dateToString({
							day: 1,
							month: month + 1,
							year: year
						});
						...
						var end = $author$project$DateUtils$dateToString({
							day: endDay,
							month: month + 1,
							year: year
						});
						return {
							endDate: $elm$core$Maybe$Just(end),
							startDate: $elm$core$Maybe$Just(start)
						};
					} else {
						return { endDate: $elm$core$Maybe$Nothing, startDate: $elm$core$Maybe$Nothing };
					}
				...
				if (_v6.b && _v6.b.b) {
					...
					var day1Str = _v7.a;
					var rest = _v7.b;
					var day1Parts = A2($elm$core$String$split, '-', day1Str);
					var _v8 = _Utils_Tuple2(
						A2(
							$elm$core$Dict$get,
							$elm$core$String$toLower(monthName),
							$author$project$KMLUtils$months
						),
						A2($elm$core$Maybe$andThen, $elm$core$String$toInt, $elm$core$List$head(day1Parts))
					);
					if (_v8.a.$ === 'Just' && _v8.b.$ === 'Just') {
						...
						var day2 = (function () {
							if (day1Parts.b && day1Parts.b.b) {
								var _v10 = day1Parts.b;
								var day2Str = _v10.a;
								return A2($elm$core$Maybe$withDefault, day1, $elm$core$String$toInt(day2Str));
							} else {
								if (rest.b && !rest.b.b) {
									var day2Str = rest.a;
									return A2($elm$core$Maybe$withDefault, day1, $elm$core$String$toInt(day2Str));
	...
	var $author$project$KMLUtils$parseEventName = function (name) {
		var openParen = A2($elm$core$String$indexes, '(', name);
		var closeParen = A2($elm$core$String$indexes, ')', name);
		var _v0 = _Utils_Tuple2($elm$core$List$head(openParen), $elm$core$List$head(closeParen));
		if (_v0.a.$ === 'Just' && _v0.b.$ === 'Just') {
			var open = _v0.a.a;
			var close = _v0.b.a;
			if (_Utils_cmp(open, close) < 0) {
				var title = $elm$core$String$trim(A2($elm$core$String$left, open, name));
				var dates = $elm$core$String$trim(A2($elm$core$String$dropLeft, close + 1, name));
				var country = $elm$core$String$trim(A3($elm$core$String$slice, open + 1, close, name));
				return {
					country: $elm$core$String$isEmpty(country)
						? $elm$core$Maybe$Nothing
						: $elm$core$Maybe$Just(country),
					dates: $elm$core$String$isEmpty(dates)
						? $elm$core$Maybe$Nothing
						: $elm$core$Maybe$Just(dates),
					title: title
				};
			} else {
				return { country: $elm$core$Maybe$Nothing, dates: $elm$core$Maybe$Nothing, title: name };
			}
		} else {
			return { country: $elm$core$Maybe$Nothing, dates: $elm$core$Maybe$Nothing, title: name };
		}
	};
	var $author$project$KMLUtils$processRawKML = function (raw) {
		var year = 2025;
		var parsed = $author$project$KMLUtils$parseEventName(raw.name);
		var location = (function () {
			var _v2 = parsed.country;
			if (_v2.$ === 'Just') {
				var c = _v2.a;
				return A2(
					$elm$core$Maybe$withDefault,
					c,
					A2($elm$core$Dict$get, c, $author$project$KMLUtils$countryMap)
				);
			} else {
				return '';
			}
		...
		var dates = (function () {
			var _v1 = parsed.dates;
			if (_v1.$ === 'Just') {
				var d = _v1.a;
				return A2($author$project$KMLUtils$parseDateString, d, year);
			} else {
				return { endDate: $elm$core$Maybe$Nothing, startDate: $elm$core$Maybe$Nothing };
			}
		})();
		var _v0 = dates.startDate;
		if (_v0.$ === 'Just') {
			var start = _v0.a;
			return $elm$core$Maybe$Just({
				allDay: true,
				created: '',
				description: $elm$core$Maybe$Just(raw.description),
				endDate: A2($elm$core$Maybe$map, $author$project$DateUtils$localDateToUTC, dates.endDate),
				id: '',
				image: $elm$core$Maybe$Nothing,
				imageDescription: $elm$core$Maybe$Nothing,
				location: $elm$core$String$isEmpty(location)
					? $elm$core$Maybe$Nothing
					: $elm$core$Maybe$Just(location),
				point:
					!!raw.lat || !!raw.lon
						? $elm$core$Maybe$Just({ lat: raw.lat, lon: raw.lon })
						: $elm$core$Maybe$Nothing,
				startDate: $author$project$DateUtils$localDateToUTC(start),
				state: $author$project$Types$Draft,
				title: parsed.title,
				updated: '',
	...
	var $author$project$KMLUtils$RawKMLData = F4(function (name, description, lat, lon) {
	...
	var $author$project$Ports$removeAuth = _Platform_outgoingPort('removeAuth', function ($) {
		return $elm$json$Json$Encode$null;
	});
	...
	var $author$project$Ports$storeAuth = _Platform_outgoingPort('storeAuth', function ($) {
		return $elm$json$Json$Encode$object(
			_List_fromArray([
				_Utils_Tuple2(
					'token',
	...
	var $elm$url$Url$toString = function (url) {
		...
		return A3(
			...
			A3(
				...
				'?',
				url.query,
				_Utils_ap(A2($elm$url$Url$addPort, url.port_, _Utils_ap(http, url.host)), url.path)
			)
		);
	};
	var $author$project$Calendar$daysInMonthsBefore = function (month) {
		switch (month.$) {
			case 'Jan':
				return 0;
			case 'Feb':
	...
	var $author$project$Calendar$isLeapYear = function (year) {
		return (
			...
			(!!A2($elm$core$Basics$modBy, 100, year) || !A2($elm$core$Basics$modBy, 400, year))
		);
	};
	var $author$project$Calendar$monthToInt = function (month) {
		switch (month.$) {
			case 'Jan':
				return 0;
			case 'Feb':
	...
	var $author$project$Calendar$daysSince1970 = F3(function (year, month, day) {
		var years = year - 1970;
		var leapYears = ((years / 4) | 0) - ((years / 100) | 0) + ((years / 400) | 0);
		var daysInYears = years * 365 + leapYears;
		var daysInMonths =
			$author$project$Calendar$daysInMonthsBefore(month) +
			($author$project$Calendar$monthToInt(month) > 1 && $author$project$Calendar$isLeapYear(year)
				? 1
				: 0);
		return daysInYears + daysInMonths + day - 1;
	});
	var $author$project$Calendar$dateToPosix = F3(function (year, month, day) {
		var daysSinceEpoch = A3($author$project$Calendar$daysSince1970, year, month, day);
		var millis = daysSinceEpoch * 24 * 60 * 60 * 1000;
		return $elm$time$Time$millisToPosix(millis);
	});
	var $author$project$Calendar$daysInMonth = F2(function (year, month) {
		switch (month.$) {
			case 'Feb':
				return $author$project$Calendar$isLeapYear(year) ? 29 : 28;
			case 'Apr':
				return 30;
			case 'Jun':
				return 30;
	...
	var $author$project$Calendar$intToMonth = function (_int) {
		switch (_int) {
			case 0:
	...
	var $elm$time$Time$toCivil = function (minutes) {
		...
		var yearOfEra =
			((dayOfEra - ((dayOfEra / 1460) | 0) + ((dayOfEra / 36524) | 0) - ((dayOfEra / 146096) | 0)) /
				365) |
			0;
	...
	var $author$project$Calendar$addMonths = F2(function (months, posix) {
		var year = A2($elm$time$Time$toYear, $elm$time$Time$utc, posix);
		var month = A2($elm$time$Time$toMonth, $elm$time$Time$utc, posix);
		var totalMonths = $author$project$Calendar$monthToInt(month) + months;
		var newMonthInt = A2($elm$core$Basics$modBy, 12, totalMonths);
		var newMonth = $author$project$Calendar$intToMonth(newMonthInt);
		var newYear = year + ((totalMonths / 12) | 0);
		var daysInNewMonth = A2($author$project$Calendar$daysInMonth, newYear, newMonth);
		var day = A2($elm$time$Time$toDay, $elm$time$Time$utc, posix);
		var newDay = A2($elm$core$Basics$min, day, daysInNewMonth);
		return A3($author$project$Calendar$dateToPosix, newYear, newMonth, newDay);
	});
	var $author$project$Calendar$update = F2(function (msg, model) {
		switch (msg.$) {
			case 'SetView':
				var newView = msg.a;
				return _Utils_update(model, { view: newView });
			case 'SetDate':
				var date = msg.a;
				return _Utils_update(model, { currentDate: date });
			case 'SetEvents':
				var events = msg.a;
				return _Utils_update(model, { events: events });
			case 'AddEvent':
				var event = msg.a;
			...
			case 'Next':
				return _Utils_update(model, {
					currentDate: A2($author$project$Calendar$addMonths, 1, model.currentDate)
				});
			default:
				return _Utils_update(model, {
					currentDate: A2($author$project$Calendar$addMonths, -1, model.currentDate)
	...
	var $author$project$EventForm$CreateEvent = function (a) {
		return { $: 'CreateEvent', a: a };
	};
	var $author$project$EventForm$UpdateEvent = F2(function (a, b) {
		return { $: 'UpdateEvent', a: a, b: b };
	});
	var $author$project$EventForm$Edit = function (a) {
		return { $: 'Edit', a: a };
	};
	var $author$project$EventForm$fromEvent = function (event) {
		return {
			allDay: event.allDay,
			description: A2($elm$core$Maybe$withDefault, '', event.description),
			endDate: A2($elm$core$Maybe$withDefault, '', event.endDate),
			errors: _List_Nil,
			image: event.image,
			imageDescription: A2($elm$core$Maybe$withDefault, '', event.imageDescription),
			loading: false,
			location: A2($elm$core$Maybe$withDefault, '', event.location),
			mode: $author$project$EventForm$Edit(event.id),
			point: event.point,
			startDate: event.startDate,
			state: event.state,
			title: event.title,
			url: A2($elm$core$Maybe$withDefault, '', event.url)
		};
	};
	...
	var $author$project$EventForm$toEvent = function (model) {
		return {
			allDay: model.allDay,
			created: '',
			description: $elm$core$Maybe$Just(model.description),
			endDate: $elm$core$String$isEmpty(model.endDate)
				? $elm$core$Maybe$Nothing
				: $elm$core$Maybe$Just(model.endDate),
			id: '',
			image: model.image,
			imageDescription: $elm$core$String$isEmpty(model.imageDescription)
				? $elm$core$Maybe$Nothing
				: $elm$core$Maybe$Just(model.imageDescription),
			location: $elm$core$String$isEmpty(model.location)
				? $elm$core$Maybe$Nothing
				: $elm$core$Maybe$Just(model.location),
			point: model.point,
			startDate: model.startDate,
			state: model.state,
			title: model.title,
			updated: '',
			url: $elm$core$String$isEmpty(model.url)
				? $elm$core$Maybe$Nothing
				: $elm$core$Maybe$Just(model.url)
		};
	};
	var $author$project$EventForm$addError = F3(function (condition, error, errors) {
	...
	var $author$project$EventForm$validate = function (model) {
		...
		return _Utils_update(model, { errors: errors });
	};
	var $author$project$EventForm$update = F2(function (msg, model) {
		switch (msg.$) {
			case 'SetTitle':
				var title = msg.a;
				return _Utils_Tuple2(_Utils_update(model, { title: title }), $elm$core$Maybe$Nothing);
			case 'SetStartDate':
				var startDate = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { startDate: startDate }),
					$elm$core$Maybe$Nothing
				);
			case 'SetEndDate':
				var endDate = msg.a;
				return _Utils_Tuple2(_Utils_update(model, { endDate: endDate }), $elm$core$Maybe$Nothing);
			case 'SetAllDay':
				var allDay = msg.a;
				return _Utils_Tuple2(_Utils_update(model, { allDay: allDay }), $elm$core$Maybe$Nothing);
			case 'SetLocation':
				var location = msg.a;
				return _Utils_Tuple2(_Utils_update(model, { location: location }), $elm$core$Maybe$Nothing);
			case 'SetDescription':
				var description = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { description: description }),
					$elm$core$Maybe$Nothing
				);
			case 'SetUrl':
				var url = msg.a;
				return _Utils_Tuple2(_Utils_update(model, { url: url }), $elm$core$Maybe$Nothing);
			case 'SetImage':
				var image = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, {
			...
			case 'SetImageDescription':
				...
				return _Utils_Tuple2(
					_Utils_update(model, { imageDescription: imageDescription }),
					$elm$core$Maybe$Nothing
				);
			case 'SetState':
				var state = msg.a;
				return _Utils_Tuple2(_Utils_update(model, { state: state }), $elm$core$Maybe$Nothing);
			case 'SetPoint':
				var point = msg.a;
				return _Utils_Tuple2(_Utils_update(model, { point: point }), $elm$core$Maybe$Nothing);
			case 'Submit':
				var newModel = $author$project$EventForm$validate(model);
				if ($elm$core$List$isEmpty(newModel.errors)) {
					var _v1 = model.mode;
					if (_v1.$ === 'Create') {
						return _Utils_Tuple2(
							_Utils_update(newModel, { loading: true }),
							$elm$core$Maybe$Just(
								$author$project$EventForm$CreateEvent($author$project$EventForm$toEvent(newModel))
							)
						);
					} else {
						var id = _v1.a;
						return _Utils_Tuple2(
							...
							$elm$core$Maybe$Just(
								A2(
									$author$project$EventForm$UpdateEvent,
									id,
									$author$project$EventForm$toEvent(newModel)
								)
							)
						);
					}
				} else {
					return _Utils_Tuple2(newModel, $elm$core$Maybe$Nothing);
				}
			case 'Validate':
				return _Utils_Tuple2($author$project$EventForm$validate(model), $elm$core$Maybe$Nothing);
			case 'LoadEvent':
				var event = msg.a;
				return _Utils_Tuple2($author$project$EventForm$fromEvent(event), $elm$core$Maybe$Nothing);
			default:
				var loading = msg.a;
				return _Utils_Tuple2(_Utils_update(model, { loading: loading }), $elm$core$Maybe$Nothing);
		}
	});
	var $author$project$EventList$FileLoaded = function (a) {
		return { $: 'FileLoaded', a: a };
	};
	var $author$project$EventList$FileSelected = function (a) {
	...
	var $author$project$EventList$update = F2(function (msg, model) {
		switch (msg.$) {
			case 'EditEvent':
			...
			case 'SetPage':
				var page = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { currentPage: page }),
					$elm$core$Platform$Cmd$none
				);
			case 'ToggleImportModal':
				return _Utils_Tuple2(
					_Utils_update(model, { showImportModal: !model.showImportModal }),
					$elm$core$Platform$Cmd$none
				);
			case 'NoOp':
				return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			case 'SelectKMLFile':
				return _Utils_Tuple2(
					...
					A2(
						...
						_List_fromArray(['application/vnd.google-earth.kml+xml', '.kml']),
						$author$project$EventList$FileSelected
					)
				);
			case 'FileSelected':
				var file = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { showImportModal: false }),
					A2(
						$elm$core$Task$perform,
	...
	var $elm$json$Json$Encode$list = F2(function (func, entries) {
		return _Json_wrap(
			A3($elm$core$List$foldl, _Json_addEntry(func), _Json_emptyArray(_Utils_Tuple0), entries)
		);
	});
	var $author$project$Ports$initMap = _Platform_outgoingPort('initMap', function ($) {
		return $elm$json$Json$Encode$object(
			_List_fromArray([
				_Utils_Tuple2(
					'center',
					(function ($) {
						...
						return A2(
							...
							$elm$core$Basics$identity,
							_List_fromArray([$elm$json$Json$Encode$float(a), $elm$json$Json$Encode$float(b)])
						);
					})($.center)
				),
				_Utils_Tuple2(
				...
				_Utils_Tuple2('zoom', $elm$json$Json$Encode$int($.zoom))
			])
		);
	});
	var $author$project$Ports$updateMap = _Platform_outgoingPort('updateMap', function ($) {
		return $elm$json$Json$Encode$object(
			_List_fromArray([
				_Utils_Tuple2(
					'center',
	...
	var $author$project$Map$update = F2(function (msg, model) {
		switch (msg.$) {
			case 'InitMap':
				return _Utils_Tuple2(
					model,
			...
			case 'UpdateMap':
				...
				var zoom = msg.b;
				var marker = msg.c;
				var newModel = _Utils_update(model, { center: center, marker: marker, zoom: zoom });
				return _Utils_Tuple2(
					newModel,
					$author$project$Ports$updateMap({ center: center, marker: marker, zoom: zoom })
				);
			default:
				var pos = msg.a;
				return _Utils_Tuple2(
		...
		switch (msg.$) {
			case 'LinkClicked':
				...
				if (urlRequest.$ === 'Internal') {
					...
					return _Utils_Tuple2(
						model,
						A2($elm$browser$Browser$Navigation$pushUrl, model.key, $elm$url$Url$toString(url))
					);
				} else {
			...
			case 'UrlChanged':
				...
				var newModel = _Utils_update(model, { route: newRoute, url: url });
				switch (newRoute.$) {
					case 'CreateEvent':
						return _Utils_Tuple2(
							_Utils_update(newModel, { eventForm: $author$project$EventForm$init }),
							$elm$core$Platform$Cmd$none
						);
					case 'EditEvent':
						var id = newRoute.a;
						...
						if (_v3.$ === 'Just') {
							var event = _v3.a;
							var _v4 = A2(
								$author$project$EventForm$update,
								$author$project$EventForm$LoadEvent(event),
								model.eventForm
							);
							var updatedEventForm = _v4.a;
							return _Utils_Tuple2(
								_Utils_update(newModel, { eventForm: updatedEventForm }),
								$elm$core$Platform$Cmd$none
							);
						} else {
					...
					case 'Callback':
						var _v5 = $author$project$Main$parseCallbackParams(
						...
						if (_v5.$ === 'Just') {
							...
							return _Utils_Tuple2(
								newModel,
								A3(
									$author$project$PocketBase$authWithOAuth2Code,
									code,
									state,
									$author$project$Main$AuthCallbackResult
								)
							);
						} else {
							return _Utils_Tuple2(newModel, $elm$core$Platform$Cmd$none);
			...
			case 'EventsMsg':
				...
				var updatedEventForm = (function () {
					switch (eventsMsg.$) {
						case 'EventCreated':
							return A2(
								$author$project$EventForm$update,
								$author$project$EventForm$SetLoading(false),
								model.eventForm
							).a;
						case 'EventUpdated':
							return A2(
						...
						switch (eventsMsg.$) {
							...
							case 'EventCreated':
								if (eventsMsg.a.$ === 'Ok') {
									...
									return A2(
										$author$project$Calendar$update,
										$author$project$Calendar$AddEvent(event),
										model.calendar
									);
								} else {
									break _v10$3;
							...
							case 'EventUpdated':
								if (eventsMsg.a.$ === 'Ok') {
									...
									return A2(
										$author$project$Calendar$update,
										$author$project$Calendar$UpdateEvent(event),
										model.calendar
									);
								} else {
									break _v10$3;
				...
				var newLoading = (function () {
					if (eventsMsg.$ === 'EventsFetched') {
						return false;
					} else {
						return model.loading;
					}
				})();
				var _v7 = A2($author$project$Events$update, eventsMsg, model.events);
				var updatedEvents = _v7.a;
				var eventsCmd = _v7.b;
				var newError = updatedEvents.error;
				return _Utils_Tuple2(
					_Utils_update(model, {
			...
			case 'MapMsg':
				...
				var mapCmd = _v12.b;
				return _Utils_Tuple2(
					_Utils_update(model, { map: updatedMap }),
			...
			case 'EventFormMsg':
							...
							return A2(
								...
								$elm$core$Task$succeed(
									$author$project$Main$EventsMsg(
										A2(
											$author$project$Events$CreateEvent,
											A2(
				...
				return _Utils_Tuple2(_Utils_update(model, { eventForm: updatedEventForm }), cmd);
			case 'EventListMsg':
				var eventListMsg = msg.a;
				switch (eventListMsg.$) {
					case 'DeleteEvent':
					...
					case 'FileLoaded':
						...
						return _Utils_Tuple2(model, $author$project$Ports$parseKMLContent(content));
					default:
						var _v17 = A2($author$project$EventList$update, eventListMsg, model.eventList);
						var updatedEventList = _v17.a;
						var cmd = _v17.b;
						return _Utils_Tuple2(
							_Utils_update(model, { eventList: updatedEventList }),
							A2($elm$core$Platform$Cmd$map, $author$project$Main$EventListMsg, cmd)
						);
				}
			case 'EventDetailMsg':
				var eventDetailMsg = msg.a;
				switch (eventDetailMsg.$) {
					case 'EditEvent':
			...
			case 'KMLParsed':
				...
				if (_v19.$ === 'Ok') {
					...
					var events = A2(
						$elm$core$List$filterMap,
						$author$project$KMLUtils$processRawKML,
						rawList
					);
					var cmds = A2(
						$elm$core$List$map,
			...
			case 'AuthRemoved':
				return _Utils_Tuple2(
					_Utils_update(model, { auth: $elm$core$Maybe$Nothing }),
					$elm$core$Platform$Cmd$none
				);
			case 'MapMarkerMoved':
				var pos = msg.a;
				var _v20 = A2($author$project$Map$update, $author$project$Map$MarkerMoved(pos), model.map);
			...
			case 'LoginResult':
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(model, {
							error: $elm$core$Maybe$Just($author$project$Main$httpErrorToString(err)),
							loading: false
						}),
						$elm$core$Platform$Cmd$none
					);
			...
			case 'UpdateLoginEmail':
				var email = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { loginEmail: email }),
					$elm$core$Platform$Cmd$none
				);
			case 'UpdateLoginPassword':
				var password = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { loginPassword: password }),
					$elm$core$Platform$Cmd$none
				);
			case 'SetDate':
				var dateStr = msg.a;
				...
				return _Utils_Tuple2(
					_Utils_update(model, {
						calendar: A2(
							$author$project$Calendar$update,
							$author$project$Calendar$SetDate(posix),
							model.calendar
						),
	...
	var $author$project$Main$EventDetailMsg = function (a) {
		return { $: 'EventDetailMsg', a: a };
	};
		...
		var attributes = _Utils_ap(
			...
			_Utils_ap(
				(function () {
					...
					if (_v0.$ === 'Just') {
						var label = _v0.a;
						return _List_fromArray([A2($elm$html$Html$Attributes$attribute, 'aria-label', label)]);
					} else {
						return _List_Nil;
					}
				})(),
				(function () {
					...
					if (_v1.$ === 'Just') {
						var msg = _v1.a;
						return _List_fromArray([$elm$html$Html$Events$onClick(msg)]);
					} else {
						return _List_Nil;
					}
				})()
	...
	var $author$project$Calendar$addDays = F2(function (days, posix) {
		var millis =
			$elm$time$Time$posixToMillis(posix) + $elm$core$Basics$round(days * 24 * 60 * 60 * 1000);
		return $elm$time$Time$millisToPosix(millis);
	});
	...
	var $elm$time$Time$toWeekday = F2(function (zone, time) {
		var _v0 = A2(
			...
			7,
			A2($elm$time$Time$flooredDiv, A2($elm$time$Time$toAdjustedMinutes, zone, time), 60 * 24)
		);
		switch (_v0) {
			case 0:
	...
	var $author$project$Calendar$weekdayToInt = function (weekday) {
		switch (weekday.$) {
			case 'Mon':
				return 1;
			case 'Tue':
	...
	var $author$project$Calendar$endOfWeek = F2(function (posix, firstDay) {
		var weekday = A2($elm$time$Time$toWeekday, $elm$time$Time$utc, posix);
		var weekdayInt = $author$project$Calendar$weekdayToInt(weekday);
		var offset = A2($elm$core$Basics$modBy, 7, firstDay - weekdayInt + 6);
		var daysForward = offset === 6 ? 6 : offset;
		return A2($author$project$Calendar$addDays, daysForward, posix);
	});
	var $author$project$Calendar$firstOfMonthPosix = F2(function (year, month) {
		return A3($author$project$Calendar$dateToPosix, year, month, 1);
	});
	var $author$project$Calendar$generateDaysHelper = F4(
		function (currentMillis, endMillis, currentMonth, acc) {
			generateDaysHelper: while (true) {
				if (_Utils_cmp(currentMillis, endMillis) > 0) {
					return acc;
				} else {
					var posix = $elm$time$Time$millisToPosix(currentMillis);
					var isCurrent = _Utils_eq(
						A2($elm$time$Time$toMonth, $elm$time$Time$utc, posix),
						currentMonth
					);
					var day = { date: posix, events: _List_Nil, isCurrentMonth: isCurrent };
					var $temp$currentMillis = currentMillis + 24 * 60 * 60 * 1000,
						$temp$endMillis = endMillis,
						$temp$currentMonth = currentMonth,
						$temp$acc = A2($elm$core$List$cons, day, acc);
					currentMillis = $temp$currentMillis;
					endMillis = $temp$endMillis;
					currentMonth = $temp$currentMonth;
					acc = $temp$acc;
					continue generateDaysHelper;
				}
			}
		}
	);
	var $author$project$Calendar$generateDays = F3(function (start, end, currentMonth) {
		var startMillis = $elm$time$Time$posixToMillis(start);
		var endMillis = $elm$time$Time$posixToMillis(end);
		var days = A4(
			$author$project$Calendar$generateDaysHelper,
			startMillis,
			endMillis,
			currentMonth,
			_List_Nil
		);
		return $elm$core$List$reverse(days);
	});
	var $author$project$Calendar$lastOfMonthPosix = F2(function (year, month) {
		return A3(
			$author$project$Calendar$dateToPosix,
			year,
			month,
			A2($author$project$Calendar$daysInMonth, year, month)
		);
	});
	var $author$project$Calendar$startOfWeek = F2(function (posix, firstDay) {
		var weekday = A2($elm$time$Time$toWeekday, $elm$time$Time$utc, posix);
		var weekdayInt = $author$project$Calendar$weekdayToInt(weekday);
		var offset = A2($elm$core$Basics$modBy, 7, weekdayInt - firstDay + 7);
		var daysBack = !offset ? 0 : offset;
		return A2($author$project$Calendar$addDays, -daysBack, posix);
	});
	var $author$project$Calendar$getCalendarDays = function (model) {
		var year = A2($elm$time$Time$toYear, $elm$time$Time$utc, model.currentDate);
		var month = A2($elm$time$Time$toMonth, $elm$time$Time$utc, model.currentDate);
		var lastOfMonth = A2($author$project$Calendar$lastOfMonthPosix, year, month);
											...
											? A2(
													...
													A2(
														...
														A2(
															$elm$core$List$cons,
															z,
															A2($elm$core$List$cons, w, A2($elm$core$List$takeTailRec, n - 4, tl))
														)
													)
												)
											: A2(
	...
	var $author$project$Calendar$groupByWeeks = function (days) {
		if (!days.b) {
			return _List_Nil;
		} else {
			var week = A2($elm$core$List$take, 7, days);
			var rest = A2($elm$core$List$drop, 7, days);
			return A2($elm$core$List$cons, week, $author$project$Calendar$groupByWeeks(rest));
		}
	};
	...
	var $author$project$Calendar$viewEvent = function (event) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray([$elm$html$Html$Attributes$class('calendar-event')]),
			_List_fromArray([$elm$html$Html$text(event.title)])
		);
	};
	var $author$project$Calendar$viewDay = function (day) {
		var dayClasses = day.isCurrentMonth ? 'calendar-day current-month' : 'calendar-day other-month';
		return A2(
			$elm$html$Html$div,
			_List_fromArray([$elm$html$Html$Attributes$class(dayClasses)]),
			_List_fromArray([
				A2(
					$elm$html$Html$div,
					_List_fromArray([$elm$html$Html$Attributes$class('calendar-day-number')]),
					_List_fromArray([
						$elm$html$Html$text(
							$elm$core$String$fromInt(A2($elm$time$Time$toDay, $elm$time$Time$utc, day.date))
						)
					])
				),
				A2(
					$elm$html$Html$div,
					_List_fromArray([$elm$html$Html$Attributes$class('calendar-day-events')]),
					A2($elm$core$List$map, $author$project$Calendar$viewEvent, day.events)
				)
			])
		);
	};
	var $author$project$Calendar$viewWeek = function (days) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray([$elm$html$Html$Attributes$class('calendar-week')]),
			A2($elm$core$List$map, $author$project$Calendar$viewDay, days)
		);
	};
	...
	var $author$project$Calendar$weekdayHeader = function (weekdayInt) {
		var weekday = $author$project$Calendar$intToWeekday(weekdayInt);
		return A2(
			$elm$html$Html$div,
			_List_fromArray([$elm$html$Html$Attributes$class('calendar-weekday')]),
			_List_fromArray([$elm$html$Html$text($author$project$Calendar$weekdayToString(weekday))])
		);
	};
	var $author$project$Calendar$view = function (model) {
		var days = $author$project$Calendar$getCalendarDays(model);
		var weeks = $author$project$Calendar$groupByWeeks(days);
		return A2(
			$elm$html$Html$div,
			_List_fromArray([$elm$html$Html$Attributes$class('calendar')]),
			_List_fromArray([
				A2(
					$elm$html$Html$div,
					_List_fromArray([$elm$html$Html$Attributes$class('calendar-header')]),
					_List_fromArray([
						A2(
							$elm$html$Html$button,
							_List_fromArray([$elm$html$Html$Events$onClick($author$project$Calendar$Previous)]),
						...
						A2(
							...
							_List_Nil,
							_List_fromArray([
								$elm$html$Html$text($author$project$Calendar$monthYearString(model.currentDate))
							])
						),
						A2(
							$elm$html$Html$button,
							_List_fromArray([$elm$html$Html$Events$onClick($author$project$Calendar$Next)]),
							_List_fromArray([$elm$html$Html$text('>')])
						)
					])
				),
				A2(
					$elm$html$Html$div,
					_List_fromArray([$elm$html$Html$Attributes$class('calendar-grid')]),
					$elm$core$List$concat(
						_List_fromArray([
							_List_fromArray([
								A2(
									$elm$html$Html$div,
									_List_fromArray([$elm$html$Html$Attributes$class('calendar-week-header')]),
									A2(
										$elm$core$List$map,
	...
	var $author$project$EventDetail$Back = { $: 'Back' };
	var $author$project$EventDetail$DeleteEvent = function (a) {
		return { $: 'DeleteEvent', a: a };
	};
	var $author$project$EventDetail$EditEvent = function (a) {
		return { $: 'EditEvent', a: a };
	};
	var $author$project$DateUtils$monthToInt = function (month) {
		switch (month.$) {
			case 'Jan':
				return 1;
			case 'Feb':
	...
	var $elm$time$Time$toHour = F2(function (zone, time) {
		return A2(
			$elm$core$Basics$modBy,
			24,
			A2($elm$time$Time$flooredDiv, A2($elm$time$Time$toAdjustedMinutes, zone, time), 60)
		);
	});
	...
	var $author$project$DateUtils$formatDateInHelsinki = F2(function (utcString, allDay) {
		var posix = $author$project$DateUtils$parseUTCDate(utcString);
		var helsinkiMillis = $elm$time$Time$posixToMillis(posix) + 2 * 60 * 60 * 1000;
		var helsinkiPosix = $elm$time$Time$millisToPosix(helsinkiMillis);
		var month = A2($elm$time$Time$toMonth, $elm$time$Time$utc, helsinkiPosix);
		var year = A2($elm$time$Time$toYear, $elm$time$Time$utc, helsinkiPosix);
		var day = A2($elm$time$Time$toDay, $elm$time$Time$utc, helsinkiPosix);
		var dateStr =
			$elm$core$String$fromInt(day) +
			('.' +
				($elm$core$String$fromInt($author$project$DateUtils$monthToInt(month)) +
					('.' + $elm$core$String$fromInt(year))));
		if (allDay) {
			return dateStr;
		} else {
			var minute = A2($elm$time$Time$toMinute, $elm$time$Time$utc, helsinkiPosix);
			var hour = A2($elm$time$Time$toHour, $elm$time$Time$utc, helsinkiPosix);
			var timeStr =
	...
	var $author$project$EventDetail$view = F2(function (event, auth) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray([$elm$html$Html$Attributes$class('container mx-auto p-4')]),
			_List_fromArray([
				A2(
					$elm$html$Html$div,
					_List_fromArray([
						$elm$html$Html$Attributes$class('flex justify-between items-start mb-6')
					]),
					_List_fromArray([
						A2(
							$elm$html$Html$h1,
							_List_fromArray([$elm$html$Html$Attributes$class('text-2xl font-bold')]),
							_List_fromArray([$elm$html$Html$text(event.title)])
						),
						(function () {
							if (auth.$ === 'Just') {
								return A2(
									$elm$html$Html$div,
									_List_fromArray([$elm$html$Html$Attributes$class('flex gap-2')]),
									_List_fromArray([
										A2(
											$elm$html$Html$button,
											_List_fromArray([
												$elm$html$Html$Events$onClick(
													$author$project$EventDetail$EditEvent(event.id)
												),
										...
										A2(
											...
											_List_fromArray([
												$elm$html$Html$Events$onClick(
													$author$project$EventDetail$DeleteEvent(event.id)
												),
						...
						return A2(
							...
							_List_fromArray([
								$elm$html$Html$Attributes$src(
									'https://data.suomenpalikkayhteiso.fi/api/files/events/' +
										(event.id + ('/' + imgPath))
								),
								$elm$html$Html$Attributes$class('w-full rounded-lg mb-6')
							]),
							_List_Nil
						);
					} else {
				...
				A2(
					$elm$html$Html$p,
					_List_fromArray([$elm$html$Html$Attributes$class('mb-4 whitespace-pre-wrap')]),
					_List_fromArray([
						$elm$html$Html$text(A2($elm$core$Maybe$withDefault, '', event.description))
					])
				),
				A2(
					$elm$html$Html$div,
					_List_fromArray([$elm$html$Html$Attributes$class('grid gap-2')]),
					_List_fromArray([
						A2(
							$elm$html$Html$p,
							_List_Nil,
							_List_fromArray([
								A2(
									...
									_List_Nil,
									_List_fromArray([$elm$html$Html$text('Start: ')])
								),
								$elm$html$Html$text(
									A2($author$project$DateUtils$formatDateInHelsinki, event.startDate, event.allDay)
								)
							])
						),
						A2(
							...
							_List_fromArray([
								A2(
									$elm$html$Html$strong,
									_List_Nil,
									_List_fromArray([$elm$html$Html$text('Location: ')])
								),
								$elm$html$Html$text(A2($elm$core$Maybe$withDefault, '', event.location))
							])
						),
						(function () {
							var _v2 = event.url;
								...
								return A2(
									...
									_List_fromArray([
										A2(
											$elm$html$Html$strong,
											_List_Nil,
											_List_fromArray([$elm$html$Html$text('URL: ')])
										),
										A2(
											$elm$html$Html$a,
											_List_fromArray([
				...
				A2(
					$elm$html$Html$button,
					_List_fromArray([
						$elm$html$Html$Events$onClick($author$project$EventDetail$Back),
						$elm$html$Html$Attributes$class(
							'mt-8 inline-block text-blue-600 underline bg-transparent border-0 cursor-pointer'
						)
					]),
					_List_fromArray([$elm$html$Html$text('Back to Calendar')])
				)
			])
		);
	});
	...
	var $author$project$DateTimePicker$view = function (config) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray([$elm$html$Html$Attributes$class('mb-4')]),
			_List_fromArray([
				A2(
					$elm$html$Html$label,
					_List_fromArray([
						$elm$html$Html$Attributes$class('mb-2 block font-medium text-gray-700'),
						$elm$html$Html$Attributes$for(config.id)
					]),
					_List_fromArray([$elm$html$Html$text(config.label)])
				),
				A2(
					$elm$html$Html$input,
					_List_fromArray([
			...
			_List_fromArray([
				A2(
					...
					_List_fromArray([
						A2(
							$elm$html$Html$label,
							_List_fromArray([$elm$html$Html$Attributes$for('title')]),
				...
				$author$project$DateTimePicker$view({
					allDay: model.allDay,
					disabled: false,
					id: 'startDate',
					label: 'Start Date',
					onChange: $author$project$EventForm$SetStartDate,
				...
				$author$project$DateTimePicker$view({
					...
					id: 'endDate',
					label: 'End Date',
					onChange: $author$project$EventForm$SetEndDate,
					value: model.endDate
				}),
				A2(
					$elm$html$Html$div,
					...
					_List_fromArray([
						A2(
							$elm$html$Html$label,
							_List_fromArray([$elm$html$Html$Attributes$for('allDay')]),
							_List_fromArray([$elm$html$Html$text('All Day')])
						),
						A2(
							$elm$html$Html$input,
							_List_fromArray([
				...
				A2(
					...
					_List_fromArray([
						A2(
							$elm$html$Html$label,
							_List_fromArray([$elm$html$Html$Attributes$for('location')]),
							_List_fromArray([$elm$html$Html$text('Location')])
						),
						A2(
							$elm$html$Html$input,
				...
				A2(
					...
					_List_fromArray([
						A2(
							$elm$html$Html$label,
							_List_fromArray([$elm$html$Html$Attributes$for('description')]),
				...
				A2(
					...
					_List_fromArray([
						A2(
							$elm$html$Html$label,
							_List_fromArray([$elm$html$Html$Attributes$for('url')]),
							_List_fromArray([$elm$html$Html$text('URL')])
						),
						A2(
							$elm$html$Html$input,
				...
				A2(
					...
					_List_fromArray([
						A2(
							$elm$html$Html$label,
							_List_fromArray([$elm$html$Html$Attributes$for('image')]),
							_List_fromArray([$elm$html$Html$text('Image URL')])
						),
						A2(
							$elm$html$Html$input,
							_List_fromArray([
								...
								$elm$html$Html$Attributes$value(A2($elm$core$Maybe$withDefault, '', model.image)),
				...
				A2(
					...
					_List_fromArray([
						A2(
							$elm$html$Html$label,
							_List_fromArray([$elm$html$Html$Attributes$for('imageDescription')]),
							_List_fromArray([$elm$html$Html$text('Image Description')])
						),
						A2(
							$elm$html$Html$input,
							_List_fromArray([
				...
				A2(
					...
					_List_Nil,
					_List_fromArray([
						A2($elm$html$Html$label, _List_Nil, _List_fromArray([$elm$html$Html$text('State')])),
						A2(
							$elm$html$Html$div,
							_List_Nil,
							_List_fromArray([
				...
				A2(
					...
					_List_fromArray([
						...
						$elm$html$Html$Attributes$disabled(
							!$elm$core$List$isEmpty(model.errors) || model.loading
						)
					]),
					_List_fromArray([$elm$html$Html$text(model.loading ? 'Submitting...' : 'Submit')])
				),
				A2(
					$elm$html$Html$button,
					_List_fromArray([$elm$html$Html$Events$onClick($author$project$EventForm$Validate)]),
					_List_fromArray([$elm$html$Html$text('Validate')])
				)
			])
		);
	};
	var $author$project$EventList$ToggleImportModal = { $: 'ToggleImportModal' };
	var $author$project$EventList$paginate = F3(function (page, size, list) {
		return A2($elm$core$List$take, size, A2($elm$core$List$drop, (page - 1) * size, list));
	});
	...
	var $author$project$EventList$viewEventRow = function (event) {
		return A2(
			$elm$html$Html$tr,
			_List_Nil,
			_List_fromArray([
				A2(
					$elm$html$Html$td,
					_List_fromArray([$elm$html$Html$Attributes$class('px-6 py-4 whitespace-nowrap')]),
					_List_fromArray([
						A2(
							$elm$html$Html$div,
							_List_fromArray([
						...
						A2(
							$elm$html$Html$div,
							_List_fromArray([$elm$html$Html$Attributes$class('text-sm text-gray-500')]),
							_List_fromArray([
								$elm$html$Html$text(A2($elm$core$Maybe$withDefault, '', event.location))
							])
						)
				...
				A2(
					$elm$html$Html$td,
					_List_fromArray([
						$elm$html$Html$Attributes$class('px-6 py-4 whitespace-nowrap text-sm text-gray-500')
					]),
					_List_fromArray([$elm$html$Html$text(event.startDate)])
				),
				A2(
					$elm$html$Html$td,
					_List_fromArray([$elm$html$Html$Attributes$class('px-6 py-4 whitespace-nowrap')]),
							...
							_List_fromArray([
								$elm$html$Html$Attributes$class(
									'px-2 inline-flex text-xs leading-5 font-semibold rounded-full ' +
										$author$project$EventList$statusColor(event.state)
								)
							]),
							_List_fromArray([
								$elm$html$Html$text($author$project$EventList$stateToString(event.state))
							])
						)
					])
				),
				A2(
					...
					_List_fromArray([
						A2(
							$elm$html$Html$a,
							_List_fromArray([
								$elm$html$Html$Attributes$href('/events/' + (event.id + '/edit')),
								$elm$html$Html$Attributes$class('text-indigo-600 hover:text-indigo-900 mr-4')
							]),
							_List_fromArray([$elm$html$Html$text('Edit')])
						),
						A2(
							$elm$html$Html$button,
							_List_fromArray([
								$elm$html$Html$Events$onClick($author$project$EventList$DeleteEvent(event.id)),
	...
	var $author$project$EventList$SelectKMLFile = { $: 'SelectKMLFile' };
	var $elm$html$Html$h3 = _VirtualDom_node('h3');
	var $author$project$EventList$viewImportModal = A2(
		$elm$html$Html$div,
		_List_fromArray([
			$elm$html$Html$Attributes$class('fixed inset-0 z-50 overflow-y-auto'),
				...
				_List_fromArray([
					A2(
						...
						_List_fromArray([
							...
							$elm$html$Html$Events$onClick($author$project$EventList$ToggleImportModal)
						]),
						_List_Nil
					),
					A2(
						...
						_List_fromArray([
							A2(
								...
								_List_fromArray([
									A2(
										$elm$html$Html$div,
										_List_fromArray([$elm$html$Html$Attributes$class('sm:flex sm:items-start')]),
										_List_fromArray([
											A2(
												$elm$html$Html$div,
												_List_fromArray([
												...
												_List_fromArray([
													A2(
														...
														_List_fromArray([$elm$html$Html$text('Import KML')])
													),
													A2(
														$elm$html$Html$div,
														_List_fromArray([$elm$html$Html$Attributes$class('mt-2')]),
														_List_fromArray([
															A2(
																$elm$html$Html$p,
																_List_fromArray([
																	$elm$html$Html$Attributes$class('text-sm text-gray-500 mb-4')
																]),
																_List_fromArray([
																	$elm$html$Html$text('Select a .kml file to import events.')
																])
															),
															A2(
																$elm$html$Html$div,
																...
																_List_fromArray([
																	A2(
																		...
																		_List_fromArray([
																			...
																			$elm$html$Html$Events$onClick(
																				$author$project$EventList$SelectKMLFile
																			)
																		]),
																		_List_fromArray([$elm$html$Html$text('Select File...')])
																	)
																])
															)
														])
							...
							A2(
								...
								_List_fromArray([
									A2(
										...
										_List_fromArray([$elm$html$Html$text('Cancel')])
									)
								])
							)
						])
	...
	var $author$project$EventList$SetPage = function (a) {
		return { $: 'SetPage', a: a };
	};
	var $author$project$EventList$viewPagination = F2(function (model, totalItems) {
		var totalPages = $elm$core$Basics$ceiling(totalItems / model.pageSize);
		return totalPages <= 1
			? $elm$html$Html$text('')
			: A2(
					$elm$html$Html$div,
					...
					_List_fromArray([
						A2(
							$elm$html$Html$div,
							_List_fromArray([
								$elm$html$Html$Attributes$class('flex-1 flex justify-between sm:hidden')
							]),
							_List_fromArray([
								A2(
									$elm$html$Html$button,
									_List_fromArray([
										$elm$html$Html$Events$onClick(
											$author$project$EventList$SetPage(
												A2($elm$core$Basics$max, 1, model.currentPage - 1)
											)
										),
										$elm$html$Html$Attributes$disabled(model.currentPage === 1),
										$elm$html$Html$Attributes$class(
											'relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50'
										)
									]),
									_List_fromArray([$elm$html$Html$text('Previous')])
								),
								A2(
									$elm$html$Html$button,
									_List_fromArray([
										$elm$html$Html$Events$onClick(
											$author$project$EventList$SetPage(
												A2($elm$core$Basics$min, totalPages, model.currentPage + 1)
											)
										),
										$elm$html$Html$Attributes$disabled(_Utils_eq(model.currentPage, totalPages)),
										$elm$html$Html$Attributes$class(
									...
									_List_fromArray([$elm$html$Html$text('Next')])
								)
							])
						),
						A2(
							...
							_List_fromArray([
								A2(
									...
									_List_fromArray([
										A2(
											$elm$html$Html$p,
											_List_fromArray([$elm$html$Html$Attributes$class('text-sm text-gray-700')]),
											_List_fromArray([
												$elm$html$Html$text('Showing page '),
												A2(
													$elm$html$Html$span,
													_List_fromArray([$elm$html$Html$Attributes$class('font-medium')]),
													_List_fromArray([
														$elm$html$Html$text($elm$core$String$fromInt(model.currentPage))
													])
												),
												...
												A2(
													...
													_List_fromArray([$elm$html$Html$Attributes$class('font-medium')]),
													_List_fromArray([
														$elm$html$Html$text($elm$core$String$fromInt(totalPages))
													])
												)
											])
										)
											...
											_List_fromArray([
												A2(
													...
													_List_fromArray([
														A2(
															$elm$html$Html$span,
															_List_fromArray([$elm$html$Html$Attributes$class('sr-only')]),
															_List_fromArray([$elm$html$Html$text('Previous')])
														),
														$elm$html$Html$text('<')
													])
												...
												A2(
													...
													_List_fromArray([
														...
														$elm$html$Html$Attributes$disabled(
															_Utils_eq(model.currentPage, totalPages)
														),
														$elm$html$Html$Attributes$class(
	...
	var $author$project$EventList$view = F2(function (model, events) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray([$elm$html$Html$Attributes$class('container mx-auto p-4')]),
			_List_fromArray([
				A2(
					...
					_List_fromArray([
						A2(
							$elm$html$Html$h1,
							_List_fromArray([$elm$html$Html$Attributes$class('text-2xl font-bold')]),
							_List_fromArray([$elm$html$Html$text('Events')])
						),
						A2(
							$elm$html$Html$div,
							_List_fromArray([$elm$html$Html$Attributes$class('flex gap-2')]),
							_List_fromArray([
								...
								A2(
									...
									_List_fromArray([
										$elm$html$Html$Attributes$class(
											'bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 block'
										),
										$elm$html$Html$Attributes$href('/events/create')
									]),
									_List_fromArray([$elm$html$Html$text('Add Event')])
								)
							])
						)
					])
				),
				model.showImportModal ? $author$project$EventList$viewImportModal : $elm$html$Html$text(''),
				A2(
					$elm$html$Html$div,
					_List_fromArray([
					...
					_List_fromArray([
						A2(
							...
							_List_fromArray([
								A2(
									$elm$html$Html$thead,
									_List_fromArray([$elm$html$Html$Attributes$class('bg-gray-50')]),
									_List_fromArray([
										A2(
											$elm$html$Html$tr,
											_List_Nil,
											_List_fromArray([
												...
												A2(
													...
													_List_fromArray([$elm$html$Html$text('Date')])
												),
												A2(
													$elm$html$Html$th,
													_List_fromArray([
													...
													_List_fromArray([$elm$html$Html$text('Status')])
												),
												A2(
													$elm$html$Html$th,
													_List_fromArray([
													...
													_List_fromArray([$elm$html$Html$text('Actions')])
												)
											])
										)
									])
						...
						A2($author$project$EventList$viewPagination, model, $elm$core$List$length(events))
					])
				)
			])
		);
							...
							_List_fromArray([
								A2(
									...
									_List_fromArray([
										...
										A2(
											$elm$html$Html$a,
											_List_fromArray([$elm$html$Html$Attributes$href('/map')]),
											_List_fromArray([$elm$html$Html$text('Map')])
										),
										$elm$html$Html$text(' | '),
										!_Utils_eq(model.auth, $elm$core$Maybe$Nothing)
											? A2(
													$elm$html$Html$a,
													_List_fromArray([$elm$html$Html$Attributes$href('/events')]),
													_List_fromArray([$elm$html$Html$text('Events')])
												)
											: $elm$html$Html$text(''),
										$elm$html$Html$text(' | '),
										A2(
											$elm$html$Html$a,
											_List_fromArray([$elm$html$Html$Attributes$href('/callback')]),
											_List_fromArray([$elm$html$Html$text('Callback')])
										)
									])
								),
								A2(
												...
												return A2(
													...
													_List_fromArray([
														...
														$author$project$Button$view({
															...
															children: _List_fromArray([$elm$html$Html$text('Logout')]),
															disabled: false,
													...
													_List_fromArray([
														...
														$author$project$Input$view({
															...
															onInput: $elm$core$Maybe$Just(
																$author$project$Main$UpdateLoginPassword
															),
															pattern: $elm$core$Maybe$Nothing,
														...
														$author$project$Button$view({
															ariaLabel: $elm$core$Maybe$Nothing,
															children: _List_fromArray([$elm$html$Html$text('Login')]),
															disabled: false,
								...
								(function () {
									...
									if (_v1.$ === 'Just') {
										...
										return A2(
											$elm$html$Html$div,
											_List_fromArray([A2($elm$html$Html$Attributes$style, 'color', 'red')]),
											_List_fromArray([$elm$html$Html$text(err)])
										);
									} else {
										return $elm$html$Html$text('');
									}
															...
															if (_v3.$ === 'Just') {
																return A2(
																	...
																	_List_fromArray([
																		A2(
																			$elm$html$Html$div,
																			_List_fromArray([$elm$html$Html$Attributes$class('flex-1')]),
																			_List_fromArray([
																				$author$project$Input$view({
																					accept: $elm$core$Maybe$Nothing,
																					ariaDescribedBy: $elm$core$Maybe$Nothing,
																		...
																		A2(
																			...
																			_List_fromArray([
																				...
																				$elm$html$Html$Events$onClick(
																					$author$project$Main$GoToCreateEvent
																				),
																				...
																				A2(
																					...
																					_List_fromArray([
																						$elm$html$Html$Attributes$href(
																							'mailto:suomenpalikkayhteisory@outlook.com'
																						)
																					]),
																					_List_fromArray([$elm$html$Html$text('email')])
																				),
																				$elm$html$Html$text('.')
																			])
																		),
											...
											case 'EventDetail':
												...
												if (_v4.$ === 'Just') {
													...
													return A2(
														$elm$html$Html$h1,
														_List_Nil,
														_List_fromArray([$elm$html$Html$text('Event not found: ' + id)])
													);
												}
											case 'EditEvent':
												var id = _v2.a;
												...
												return A2(
													...
													_List_fromArray([
														A2(
															$elm$html$Html$h1,
															_List_Nil,
															_List_fromArray([$elm$html$Html$text('Authentication Callback')])
														),
														A2(
															$elm$html$Html$p,
															_List_Nil,
															_List_fromArray([$elm$html$Html$text('Processing authentication...')])
														)
													])
												);
											default:
												return A2(
													...
													_List_fromArray([
														A2(
															$elm$html$Html$h1,
															_List_Nil,
															_List_fromArray([$elm$html$Html$text('404 - Page Not Found')])
														),
														A2(
															$elm$html$Html$p,
															_List_Nil,
														...
														A2(
															$elm$html$Html$a,
															_List_fromArray([$elm$html$Html$Attributes$href('/')]),
															_List_fromArray([$elm$html$Html$text('Go Home')])
														)
													])
												);
										}
