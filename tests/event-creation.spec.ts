...
test.describe('Event Creation Workflow', () => {
	test('should show login required message for non-authenticated users on events page', async ({
		page
	}) => {
		...
		await expect(
			page.locator('text=Sinun täytyy kirjautua sisään hallitaksesi tapahtumia.')
		).toBeVisible();
	});
