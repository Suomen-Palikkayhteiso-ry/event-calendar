...
<h2>${event.url ? `<a href="${event.url}" target="_blank" style="text-decoration: none; color: black;">${event.title}${event.location ? ` <span style="font-weight: normal";>| ${event.location}</span>` : ''}</a>` : `${event.title}${event.location ? ` <span style="font-weight: normal";>| ${event.location}</span>` : ''}`}</h2>
